...
def evaluate_strategy(df, X, y, prefix):
    ...
    if rf_path.exists():
        candidates.append(('saved_rf', joblib.load(rf_path)))
    if lgb_path.exists():
        candidates.append(('saved_lgbm', joblib.load(lgb_path)))

    # Baseline quick training
    rf = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
    rf.fit(X_train, y_train)
    candidates.append(('rf_trained', rf))

    lgbc = lgb.LGBMClassifier(n_estimators=100, random_state=42)
    lgbc.fit(X_train, y_train)
    candidates.append(('lgbm_trained', lgbc))
